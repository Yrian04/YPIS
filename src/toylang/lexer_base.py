...
class ToyLangLexerBase(Lexer):
    ...
    def nextToken(self):
        ...
        if self._input.LA(1) == ToyLangParser.EOF and len(self.indents) != 0:
            # Remove any trailing EOF tokens from our buffer.
            self.tokens = [token for token in self.tokens if token.type != ToyLangParser.EOF]

            # First emit an extra line break that serves as the end of the statement.
            self.emitToken(self.commonToken(ToyLangParser.NEWLINE, '\n'))
            ...
            self.emitToken(self.commonToken(ToyLangParser.EOF, '<EOF>'))

        next_ = super().nextToken()
        return next_ if len(self.tokens) == 0 else self.tokens.pop(0)

    def createDedent(self):
        return self.commonToken(ToyLangParser.DEDENT, '')
    ...
    def onNewLine(self):
        ...
        if self.opened > 0 or (next_next != -1 and next_ in (10, 13, 35)):
            ...
            self.emitToken(self.commonToken(ToyLangParser.NEWLINE, new_line))
            ...
            if indent == previous:
                ...
                self.emitToken(self.commonToken(ToyLangParser.INDENT, spaces))
            else:
